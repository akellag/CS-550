...
const char *WINDOWTITLE = { "OpenGL / GLUT Project 2 - Goda Akella" };
...
#define BLADE2_RADIUS        5.0
#define BLADE2_WIDTH         0.4

#define BLADEREAR_RADIUS     1.5
#define BLADEREAR_WIDTH      0.4

#define BLADETOP_RADIUS      5.0
#define BLADETOP_WIDTH       0.5
// initial window size:
...
Display( )
{
	...
	{
		gluLookAt(-0.4, 1.8, -4.9, 0, 0, -80, 0, 1., 0.);
	}
	...
	glShadeModel(GL_FLAT);
	SetPointLight(GL_LIGHT1, -10, -10, 0, 0., 0., 1.);
	glTranslatef(0, 0, -10);
	glColor3f(1, 0, 1);
	...
	if (WhichType == 0)
	{
		...
		glRotatef(360 * Time, 0., 1., 0.);
		glScalef(1, 1, 1);
		glCallList(Blade2List);
		glPopMatrix();*/

		glPushMatrix();
		glColor3f(0., 0.25, 0.75);
		glTranslatef(0, 2.5, 9);
		glRotatef(3*360* Time,1, 0, 0.);
		glScalef(1, 1, 1);
...
InitMenus( )
{
	...
	int typemenu = glutCreateMenu(DoTypeMenu);
...
InitLists( )
{
	...
	glutWireTorus(1, 5, 19, 60);

	glPopMatrix();
	glEndList();


	WireList2 = glGenLists(1);
	glNewList(WireList2, GL_COMPILE);
	glPushMatrix();
	glTranslatef(0, 0, -10);
	glColor3f(0., 0.25, 0.75);
	glutSolidTeapot(1);
	...
	glVertex3f(0,BLADEREAR_RADIUS, BLADEREAR_WIDTH / 2.);
	glVertex3f(0.,0 ,0);
	glVertex3f(0,BLADEREAR_RADIUS, -BLADEREAR_WIDTH / 2.);
